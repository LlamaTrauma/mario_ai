...
int main(int argc, char** argv)
{
    ...
    while(stat (("framesOut/" + std::to_string(f_name)).c_str(), &buffer) == 0){
        f_name ++;
    }

    mkdir(("framesOut/" + std::to_string(f_name)).c_str(), 0);

    Trainer trainer(f_name);

    //Initialize states
    sn::MainBus stateBus;
    sn::PictureBus statePBus;
    sn::VirtualScreen stateScreen;
    int wantedStates[] = {0, 1, 3, 5};
    for(int i = 0; i < 4; i ++){
        sn::Emulator::states.push_back(sn::state(stateBus, statePBus, stateScreen, 1, "states/state" + std::to_string(wantedStates[i])));
    }
    srand(time(0));
