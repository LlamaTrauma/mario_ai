...
sn::MainBus stateBus;
sn::PictureBus statePBus;
sn::VirtualScreen stateScreen;
sn::state sn::Emulator::savedState(stateBus, statePBus, stateScreen, 1);
...
namespace sn
{
    ...
    void Emulator::run(Cartridge& m_cartridge)
    {
        bool printAll = false;
        //int numDraws = 0;
        //int numSteps = 0;
        ...
        while(1)
        {
            ...
            //     }
            //     else if (focus && event.type == sf::Event::KeyReleased && event.key.code == sf::Keyboard::F7)
            //     {
            //         LOG(Info) << "save";
            //         saveState();
            //     }
            //     else if (focus && event.type == sf::Event::KeyReleased && event.key.code == sf::Keyboard::F8)
            //     {
            //         LOG(Info) << "load";
            //         loadState();
            //     }
    ...
    void Emulator::loadState(){
        // savedState.fromFile();
        m_bus = savedState.bus;
        m_pictureBus = savedState.pictureBus;
        m_cpu = savedState.cpu;
        m_ppu = savedState.ppu;
