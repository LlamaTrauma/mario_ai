...
namespace sn
{
    ...
    void MainBus::operator=(const MainBus& bus){
        m_RAM = bus.m_RAM;
        m_extRAM = bus.m_extRAM;
    }

    void MainBus::toFile(std::string fname){
    ...
    void MainBus::fromFile(std::string fname){
    ...
    {
        if (addr < 0x2000){
            // if(addr == CURRENTCOLUMNPOS){
            //     m_writeCallbacks[CURRENTCOLUMNPOS](value);
            // }
