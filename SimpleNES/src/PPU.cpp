...
namespace sn
{
    ...
    void PPU::toFile(std::string fname){
        std::ofstream out(fname);
        out << m_greyscaleMode << ' ';
        out << m_longSprites << ' ';
        out << m_vblank << ' ';
        out << m_showBackground << ' ';
        out << m_showSprites << ' ';
        out << m_evenFrame << ' ';
        out << m_firstWrite << ' ';
        out << static_cast<int>(m_bgPage) << ' ';
        out << static_cast<int>(m_sprPage) << ' ';
        out << m_dataAddress << ' ';
        out << m_cycle << ' ';
        out << m_scanline << ' ';
        out << m_spriteDataAddress << ' ';
        out << m_fineXScroll << ' ';
        out << m_tempAddress << ' ';
        out << m_dataAddrIncrement << ' ';
        out << m_sprZeroHit << ' ';
        out << static_cast<int>(m_pipelineState) << ' ';
        out << m_dataBuffer << ' ';
        out << m_hideEdgeBackground << ' ';
        out << m_hideEdgeSprites << ' ';
        out << m_generateInterrupt << ' ';
        for(int i = 0; i < 256; i++) {
            out << m_spriteMemory[i] << ' ';
        }
        out.close();
    }

    void PPU::fromFile(std::string fname){
        ...
        int temp;
        in >> temp;
        m_bgPage = static_cast<sn::PPU::CharacterPage>(temp);
        in >> temp;
        m_sprPage = static_cast<sn::PPU::CharacterPage>(temp);
        in >> m_dataAddress;
        in >> m_cycle;
        in >> m_scanline;
        in >> m_spriteDataAddress;
        in >> m_fineXScroll;
        in >> m_tempAddress;
        in >> m_dataAddrIncrement;
        in >> m_sprZeroHit;
        in >> temp;
        m_pipelineState = static_cast<sn::PPU::State>(temp);
        in >> m_dataBuffer;
        in >> m_hideEdgeBackground;
        in >> m_hideEdgeSprites;
        in >> m_generateInterrupt;
        for(int i = 0; i < 256; i++) {
            in >> m_spriteMemory[i];
        }
        in.close();
    ...
    void PPU::operator=(const PPU& ppu){
        m_spriteMemory = ppu.m_spriteMemory;
        m_scanlineSprites = ppu.m_scanlineSprites;
        m_pictureBuffer = ppu.m_pictureBuffer;
        m_longSprites = ppu.m_longSprites;
        m_greyscaleMode = ppu.m_greyscaleMode;
        m_vblank = ppu.m_vblank;
        m_showBackground = ppu.m_showBackground;
        m_showSprites = ppu.m_showSprites;
        m_evenFrame = ppu.m_evenFrame;
        m_firstWrite = ppu.m_firstWrite;
        m_bgPage = ppu.m_bgPage;
        m_sprPage = ppu.m_sprPage;
        m_dataAddress = ppu.m_dataAddress;
        m_cycle = ppu.m_cycle;
        m_scanline = ppu.m_scanline;
        m_spriteDataAddress = ppu.m_spriteDataAddress;
        m_fineXScroll = ppu.m_fineXScroll;
        m_tempAddress = ppu.m_tempAddress;
        m_dataAddrIncrement = ppu.m_dataAddrIncrement;
        m_sprZeroHit = ppu.m_sprZeroHit;
        m_pipelineState = ppu.m_pipelineState;
        m_dataBuffer = ppu.m_dataBuffer;
        m_hideEdgeBackground = ppu.m_hideEdgeBackground;
        m_hideEdgeSprites = ppu.m_hideEdgeSprites;
        m_generateInterrupt = ppu.m_generateInterrupt;
    }
